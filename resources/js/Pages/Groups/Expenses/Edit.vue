...
const selectedMembers = ref(
    props.expense.splits ? props.expense.splits.map(s => s.user_id) : props.group.members.map(m => m.id)
);

// Initialize custom amounts from existing splits
const customAmounts = ref({});
props.group.members.forEach(m => {
    const existingSplit = props.expense.splits?.find(s => s.user_id === m.id);
    customAmounts.value[m.id] = existingSplit && props.expense.split_type === 'custom' ? existingSplit.share_amount : '';
});

// Initialize percentages from existing splits
const percentages = ref({});
props.group.members.forEach(m => {
    const existingSplit = props.expense.splits?.find(s => s.user_id === m.id);
    percentages.value[m.id] = existingSplit && props.expense.split_type === 'percentage' ? existingSplit.percentage : '';
});
